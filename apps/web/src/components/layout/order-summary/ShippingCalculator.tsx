      ...
      {shipping && isCalculado && (
        <div className="mt-2 p-3.5 bg-zinc-50/80 rounded-xl border border-zinc-200/60 font-sans text-xs text-zinc-600 space-y-1 animate-fadeIn">
          <div className="flex justify-between items-center border-b border-zinc-200 pb-1.5 mb-1.5">
            <span className="font-bold text-zinc-900 font-chakra-petch uppercase tracking-wider text-[10px]">
              Endereço de Envio
            </span>
          ...
          <p className="text-sm font-bold text-zinc-800 font-chakra-petch leading-tight">
            {shipping?.logradouro} - {shipping?.neighborhood}
          </p>
          ...
          {freteSelecionado !== null && (
            <div className="mt-2 pt-1.5 border-t border-zinc-200 flex justify-between items-center text-zinc-700 font-chakra-petch">
              ...
              <span className="font-bold text-zinc-900">
                R$ {formatPrice(freteSelecionado)}
              </span>
            </div>
          )}
          <button
            type="button"
            onClick={() => setIsModalFreteOpen(true)}
            className="text-[#D9A128] font-bold font-chakra-petch uppercase tracking-wide text-[10px] mt-1.5 hover:underline block cursor-pointer"
          >
            Alterar frete
          </button>
        </div>
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl p-6 font-chakra-petch border border-zinc-200 shadow-xl">
            ...
            <div className="space-y-2">
              {[shipping?.sedex, shipping?.pac].map((option) => {
                ...
                return (
                  <label
                    key={`${option?.price}-${option?.deadline}`}
                    className={`flex items-center justify-between p-3.5 rounded-xl border cursor-pointer transition-all ${isSelected ? "border-[#D9A128] bg-amber-50/40 ring-1 ring-[#D9A128]" : "border-zinc-200"}`}
                  >
                    <div className="flex items-center gap-3">
                      <input
                        ...
                        checked={isSelected}
                        onChange={() =>
                          handleSelectOption(Number(option?.price))
                        }
                      ...
                      <div>
                        ...
                        <p className="text-xs text-zinc-400 font-sans">
                          Prazo: {option?.deadline} dias
                        </p>
                      </div>
                    </div>
                    <span className="text-sm font-black text-zinc-900">
                      R$ {formatPrice(Number(option?.price))}
                    </span>
                  </label>
                );
              })}
            ...
            <button
              type="button"
              onClick={() => setIsModalFreteOpen(false)}
              className="mt-6 rounded-xl bg-zinc-900 text-white font-bold text-xs uppercase tracking-wider py-3 w-full text-center cursor-pointer"
            >
              Confirmar
            </button>
          </div>
